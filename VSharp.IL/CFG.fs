...
and CfgInfo internal (cfg : CfgTemporaryData) =
    ...
    let resolveBasicBlockOffset offset = cfg.SortedOffsets.[resolveBasicBlockIndex offset]
    let resolveBasicBlock offset = cfg.SortedBasicBlocks.[resolveBasicBlockIndex offset]

    let sinks = cfg.Sinks |> Array.map resolveBasicBlockOffset
    ...
    member this.DistancesFrom offset =
        let bb = resolveBasicBlockOffset offset
