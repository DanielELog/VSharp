    ...
    member x.StepDone (steppedStates : cilState list) =
        ...
        if CilStateOperations.currentIp cilState = Exit entryPoint then x.Terminate()
            ...
            steppedStates |> List.iter (fun cilState ->
                ...
                | head::tail ->
                    let lastPush = lastPushType |> x.communicator.SerializeStackPush
            ...
            x.communicator.SendExecResponse concretizedOps internalCallResult currentStackPush stackPushInfo framesCount
            callIsSkipped <- false
